...
# Dental taxonomy codes: 1223 (Dentists), 1224 (Dentist Anesthesiologists)
DENTAL_TAXONOMY_PREFIXES = ("1223", "1224")
...
def is_dental_row(row):
    """Check if any taxonomy code column starts with a dental prefix (1223/1224)."""
    for i in range(1, 16):
        col = f"{TAXONOMY_COL_PREFIX}{i}"
        val = row.get(col, "")
        if val and any(str(val).startswith(p) for p in DENTAL_TAXONOMY_PREFIXES):
            return True
    return False
...
def get_taxonomy_specialty(code):
    ...
    if not code:
        return None
    if code in DENTAL_TAXONOMIES:
        return DENTAL_TAXONOMIES[code]
    # Only treat as general dentist if it's a recognized dental taxonomy prefix
    if any(code.startswith(p) for p in DENTAL_TAXONOMY_PREFIXES):
        return "general"
    return None

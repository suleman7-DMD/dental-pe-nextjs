...
def _sync_incremental_id(sqlite_session, pg_engine, config):
    ...
    if state and state["last_sync_value"]:
        ...
        # Also pick up updated rows (if model has updated_at)
        updated_rows = []
        if hasattr(model, "updated_at") and state.get("last_sync_ts"):
            from datetime import datetime
            try:
                last_ts = datetime.fromisoformat(state["last_sync_ts"])
                updated_rows = (
                    sqlite_session.query(model)
                    .filter(model.updated_at > last_ts, model.id <= last_id)
                    .all()
                )
                if updated_rows:
                    log.info("[%s] Found %d updated rows since last sync", table_name, len(updated_rows))
            except (ValueError, TypeError):
                pass
        # Merge: new rows + updated rows (deduplicated by id)
        seen_ids = set()
        rows = []

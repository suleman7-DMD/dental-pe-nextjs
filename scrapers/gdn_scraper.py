...
def extract_deal_blocks(soup):
    ...
    for el in content.find_all(["p", "hr", "h2", "h3", "h4", "li"]):
        ...
        # Each <li> is its own deal block when it starts with a capital letter
        if el.name == "li" and re.match(r'[A-Z]', text):
            if current_block:
                blocks.append(" ".join(current_block))
                current_block = []
            blocks.append(text)
...
def extract_target(text, platform):
    ...
    for pattern in [
        rf'advised\s+([A-Z][{_N}]{{3,50}}?)\s+(?:in its|on its|in the|in (?:her|his|their) sale)',
        ...
        rf'partner(?:ed|ship)s?\s+with\s+(?:Dr\.\s+[A-Za-z]+\s+[A-Za-z]+\s+and\s+(?:the\s+)?)?([A-Z][{_N}]{{3,50}}?)(?:\s+team|\s+in\s|,|\.|;|\(|\s+and\s)',
...
def run(dry_run=False):
    ...
    try:
        ...
        log.error("GDN scraper failed: %s", e)
        log_scrape_error("gdn_scraper", str(e), _t0)
        raise
    finally:
        if session:
            session.close()

...
def _location_match_pass(session, practices, counts, dry_run, force, today):
    ...
    for practice in candidates:
        ...
        if not dry_run:
            ...
            practice.classification_reasoning = f"Location-matched to {dso_name} at {best_loc.address} (fuzzy score={best_score})"
            if pe_sponsor:
...
def _classify_single_entity(row, addr_group, shared_eins, shared_phones):
    ...
    # ── Rule 1: Non-clinical ──
    # Skip non_clinical classification if already known to be DSO-affiliated
    ownership = row.get("ownership_status")
    if ownership not in ("dso_affiliated", "pe_backed"):

...
KNOWN_PLATFORMS = [
    ...
    "Rodeo Dental", "Smile Brands", "Ideal Dental", "Jefferson Dental",
    "Pine Dentistry & Braces", "DecisionOne Dental", "Gentle Dental",
    "Coast Dental", "DentalOne Partners", "Perfect Teeth",
    "Brighter Dental", "Castle Dental", "American Dental Partners",
    "Affordable Dentures", "Refresh Dental", "Dental Depot",
...
def split_into_deal_sentences(paragraphs):
    ...
    for para in paragraphs:
        ...
        protected = para
        for orig, repl in _ABBREV_PROTECT:
...
def is_international(text):
    """Check if a deal sentence describes an international (non-US) deal."""
    t = text.lower()
...
def is_credit_news(text):
    """Check if a sentence is about credit/debt/ratings, not an actual deal."""
    t = text.lower()
...
def _is_deal_sentence(text):
    ...
    t = text.lower()
    # Skip international and credit/debt news
    if is_international(text) or is_credit_news(text):
        return False
...
def _match_known_sponsor(candidate):
    """Check if candidate string contains a known PE sponsor (unidirectional)."""
    if len(candidate) < 4:
        return None
    for s in sorted(KNOWN_PE_SPONSORS, key=len, reverse=True):
...
def run(dry_run=False):
    ...
    if not valid_urls:
        ...
        log_scrape_complete("pesp_scraper", _t0, new_records=0, summary="No valid PESP pages found")
        return
    ...
    try:
        ...
        log.error("PESP scraper failed: %s", e)
        log_scrape_error("pesp_scraper", str(e), _t0)
        raise
    finally:
        if session:
            session.close()

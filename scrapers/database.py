...
class DSOLocation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("dso_name", "address", "zip", name="uix_dso_location"),
    )
...
# Module-level engine cache to avoid creating new engines on every call
_engine_cache = {}
...
def log_practice_change(session: Session, **kwargs):
    """Record a change in practice_changes (with 30-day dedup)."""
    from datetime import timedelta
    npi = kwargs.get("npi")
    field = kwargs.get("field_changed")
    new_val = kwargs.get("new_value")
    change_date = kwargs.get("change_date") or date.today()
    # Dedup: skip if identical change was logged within last 30 days
    recent_cutoff = change_date - timedelta(days=30)
    existing = session.query(PracticeChange).filter(
        PracticeChange.npi == npi,
        PracticeChange.field_changed == field,
        PracticeChange.new_value == new_val,
        PracticeChange.change_date >= recent_cutoff,
    ).first()
    if existing:
        log.debug("Duplicate change skipped for NPI %s: %s → %s (already logged)", npi, field, new_val)
        return
    change = PracticeChange(**kwargs)
    session.add(change)
    session.commit()
    log.info("Logged change for NPI %s: %s", npi, field)

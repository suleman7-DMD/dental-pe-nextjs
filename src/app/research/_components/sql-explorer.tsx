  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-[#8892A0]">
        <span className="text-[#E8ECF1] font-medium">SQL Explorer</span> -- query the database
      ...
      <div className="flex flex-wrap gap-2">
        {SQL_PRESETS_ROW1.map((preset) => (
          <button
            ...
            className="rounded-md border border-[#1E2A3A] bg-[#141922] px-3 py-1.5 text-xs text-[#8892A0] hover:text-[#E8ECF1] hover:border-[#2A3A4A] transition-colors"
          >
            {preset.name}
          </button>
        ))}
      ...
      <textarea
        ...
        className="w-full h-[150px] rounded-md border border-[#1E2A3A] bg-[#141922] text-[#E8ECF1] px-3 py-2 text-sm font-mono resize-y focus:outline-none focus:ring-2 focus:ring-[#0066FF] placeholder:text-[#4A5568]"
      />
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          disabled={loading}
          className="flex items-center gap-1.5 rounded-md bg-[#0066FF] px-4 py-2 text-sm font-medium text-white hover:bg-[#0052CC] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading ? (
        ...
        {result && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-md border border-[#1E2A3A] bg-[#141922] px-3 py-2 text-sm text-[#8892A0] hover:text-[#E8ECF1] hover:border-[#2A3A4A] transition-colors"
          >
                ...
                {result.rows.map((row, rowIdx) => (
                  ...
                  >
                    {result.columns.map((col) => (
                      <td
                        key={col}
                        className="px-4 py-2 text-[#E8ECF1] max-w-[300px] truncate text-xs"

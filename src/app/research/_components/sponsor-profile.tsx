        ...
        <>
          ...
          {timelineData.length > 0 && (
            <div className="rounded-[10px] border border-[#E8E5DE] bg-[#FFFFFF] p-4">
              <h3 className="text-sm font-medium text-[#1A1A1A] mb-3">Deal Timeline</h3>

              {/* Legend */}
              <div className="flex flex-wrap gap-3 mb-4">
                {dealTypes.map((t) => (
                  <span key={t} className="flex items-center gap-1.5 text-xs text-[#6B6B60]">
                    <span
                      className="inline-block h-2.5 w-2.5 rounded-full"
                      style={{ backgroundColor: TYPE_COLORS[t] ?? '#9C9C90' }}
                    />
                    {t}
                  </span>
                ))}
                      ...
                      return (
                        <div
                          ...
                          style={{
                            left: `${x}%`,
                            top: `${y}%`,
                            backgroundColor: TYPE_COLORS[d.type] ?? '#9C9C90',
                          }}
          ...
          {platforms.length > 0 && (
            <div className="rounded-[10px] border border-[#E8E5DE] bg-[#FFFFFF] overflow-hidden">
              <div className="px-4 py-3 border-b border-[#E8E5DE]">
                <h3 className="text-sm font-medium text-[#1A1A1A]">Platforms</h3>
              </div>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {platforms.map((p) => (
                    ...
                    >
                      <td className="px-4 py-2 text-[#1A1A1A]">{p.platform}</td>
                      <td className="px-4 py-2 text-[#6B6B60] font-mono">{p.count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {recent.length > 0 && (
            <div className="rounded-[10px] border border-[#E8E5DE] bg-[#FFFFFF] overflow-hidden">
              <div className="px-4 py-3 border-b border-[#E8E5DE]">
                <h3 className="text-sm font-medium text-[#1A1A1A]">Recent Activity</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>

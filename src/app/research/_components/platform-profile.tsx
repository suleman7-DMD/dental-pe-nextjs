...
export function PlatformProfile({ platforms }: PlatformProfileProps) {
  ...
  const STATE_COLORS = useMemo(() => {
    const palette = [
      '#0066FF', '#00C853', '#AA00FF', '#FF6D00', '#00BCD4',
      '#F44336', '#FFB300', '#78909C', '#7C4DFF', '#E91E63',
        ...
        <>
          ...
          {timelineData.length > 0 && (
            <div className="rounded-[10px] border border-[#1E2A3A] bg-[#141922] p-4">
              <h3 className="text-sm font-medium text-[#E8ECF1] mb-3">Deal Timeline by State</h3>

              {/* Legend */}
              <div className="flex flex-wrap gap-3 mb-4">
                {uniqueStates.map((s) => (
                  <span key={s} className="flex items-center gap-1.5 text-xs text-[#8892A0]">
                    <span
          ...
          {deals.length > 0 && (
            <div className="rounded-[10px] border border-[#1E2A3A] bg-[#141922] overflow-hidden">
              <div className="px-4 py-3 border-b border-[#1E2A3A]">
                <h3 className="text-sm font-medium text-[#E8ECF1]">All Deals</h3>
              </div>
              <div className="overflow-x-auto">
                  ...
                  <tbody>
                    {deals.map((d, idx) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 text-[#8892A0]">{d.specialty ?? '--'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

...
src/
  ...
  components/             Shared UI components
    ...
    data-display/         DataTable, KPI cards, badges, confidence stars, corporate-band-bar (3-anchor floor→ADA band)
...
> **2026-05-30 reclassification + IL DSO seeding — numbers below are post-audit, SYNCED to Supabase.** Two passes: (1) location-level reclassifier rewrite (`scrapers/reclassify_locations.py`) set a 4.02% floor (200/4,970); (2) an extensive zero-fabrication IL DSO hunt (NPPES brand-mining + web-verified friendly-PC clusters + DSO public locators) promoted 62 watched-IL GP locations independent → corporate, raising the confirmed corporate floor to **262 / 4,970 GP locations = 5.27%** (CHI 243/4,608 = 5.27%, BOS 19/362 = 5.25%). **The location-level floor tables (zip_scores, practice_locations, dso_locations) ARE synced to Supabase via `scrapers/_sync_floor_tables_only.py` — the LIVE site shows 5.27%.** `getCorporateBand(confirmedPct, state)` in `consolidation-honesty.ts` takes the floor as a runtime parameter from `zip_scores`, so the band auto-reflects the synced value (no hardcoded number). NPI-level `practices` flips (Job Market NPI-unit scoring) are NOT yet synced — Supabase NPI corporate stays 875/6.33% (SQLite is 1,089/7.88%) until the next weekly full sync. See project-root `CLAUDE.md` F29 cheat-sheet for the full reconciliation.

> **2026-06-07 — three-anchor band component shipped.** `src/components/data-display/corporate-band-bar.tsx` (`CorporateBandBar`) is now the canonical VISUAL for corporate consolidation: a proportional track with three markers — per-LOCATION floor (5.27%, red) → per-DENTIST floor (IL 9.68%, red — our confirmed corporate re-counted by dentist via the NPI-level `practices` classifier; ~80% same NPI set as the location floor (690/861), lift primarily office-density) → ADA HPI per-dentist anchor (IL 14.6%, goldenrod), with two hatched gap segments (red = density effect; amber = genuinely unmeasured local-name DSOs). Presentational only — reads `CorporateBand` from `consolidation-honesty.ts`, never hardcodes a %. Wired into Market Intel → Consolidation tab (`market-intel-shell.tsx`, replaced the old static two-line legend); Home / Job Market / Warroom Sitrep keep using `corporateBandSubtitle`/`corporateBandTooltip` which auto-upgraded to surface the per-dentist bridge. `consolidation-honesty.ts` gained `CONFIRMED_PER_DENTIST_CORPORATE` (IL 754/7,792=9.68%, MA 73/1,752=4.17%) and `getCorporateBand` now returns the per-dentist anchor. **The unit reframe is the point: ADA ~15% is per DENTIST, our floor is per-LOCATION; corporate offices employ ~2× the dentists (≈3.3 vs ≈1.4/loc in IL), so 5.27%→9.68% is primarily density (the NPI-level and location-level classifiers are independent and agree on ~80% of corporate dentists, not a strict re-count) and only 9.68%→14.6% is truly-unmeasured hidden DSOs.** `npm run build` ✓, F27 vitest ✓. Tooltip uses the base-ui render-prop pattern (`<TooltipTrigger delay={200} render={<span…} />`).

> **2026-06-07 (later) — Phase-4 Data-Axle structural verification RAISED the floor 5.27% → 5.73%. The two notes above are historical; these numbers supersede them.** Re-parsing the full Data Axle records (383 cols, not the legacy ~25) surfaced corporate signals — shared EIN/TIN clusters across 3+ ZIPs, shared corporate mailing addresses, and brand strings — that name-only matching missed. After adversarial REFUTE verification + free WebSearch spot-checks (two clusters were **rejected**: EIN 363676741 Mt Prospect, and corporate-mailing DAMAIL 1730 Park St #106 60563 — single-weak-signal, no corroboration), **27 verified locations** (23 new independent→corporate promotions + 4 already-corporate corroborations) were promoted via `scrapers/reclassify_verified_corporate_il.py`, each evidence-backed in `data/dso_research/il_dso_data_axle_verified.json`. Net: **+23 location flips + 89 NPI flips**, all out of GP-independent classes (specialists stay OUT of the GP floor). New live numbers, SYNCED to Supabase (`_sync_floor_tables_only.py`, verified read-back LIVE floor 285/4970): per-LOCATION floor **285 / 4,970 GP = 5.73%** (CHI 266/4,608 = 5.77%, BOS 19/362 = 5.25%); NPI-level corporate **1,178 / 13,818 = 8.53%** in SQLite (Supabase NPI-unit stays 875/6.33% until `practices` re-syncs). `consolidation-honesty.ts` `CONFIRMED_PER_DENTIST_CORPORATE` updated to **IL 824/7,792 = 10.57%**, MA 73/1,752 = 4.17% — so the band bar now renders **5.73% → 10.57% → 14.6%** (per-LOCATION floor auto-reflects 5.73% at runtime; per-DENTIST floor is the updated constant). The two classifiers now agree on **760/931** corporate dentists (~82%) and the per-dentist set covers **760/824** (92%) of the location-floor corporate. By-brand promotions: Destiny/ProSmile ×5, Precision ×4, Heartland ×3, Family Dental Care ×3, Park Place ×3, Dental Dreams ×2, Gentle ×2, Dental 360/Brite ×2, Sonrisa/CDCA ×2, DCA ×1. Durability PROVEN: a full `merge_and_score.py` recompute held 285 (it recomputes `corporate_location_count` FROM `practice_locations.entity_classification`, which no refresh step rebuilds); the F28 FLOOR CI guard `expect_min` was bumped 262 → **285**. Full reconciliation: project-root `CLAUDE.md` F29 cheat-sheet (lines 10–11) + the 2026-06-07 Phase-4 dated note.

> **2026-06-12 adversarial Chicagoland audit — FLOOR CORRECTED DOWN 5.73% → 5.43%; Data-Axle synthetic records purged from every denominator; fraction-vs-percent display bugs fixed; Supabase fully healed. The notes above are historical; these numbers supersede them.** Three root-level data fixes landed in the pipeline repo and flowed through to Supabase: **(1) 18 false-corporate location demotions** — the "Evenly Technologies" cluster (13 locations whose only corporate signal was the `parent_iusa=000000000` Data-Axle placeholder), landlord/PE-name confusions, and bad seed rows; 3 reviewed candidates were KEPT (evidence: `data/dso_research/il_false_corporate_demotions_20260612.json` in the parent repo). **(2) 179 `da_unverified` purges** — Data-Axle-only records with synthetic `DA_`-prefixed NPIs and no federal NPI at the address, now a 13th entity class excluded from EVERY denominator; the importer was root-fixed so new ones can't enter. **(3) Per-ZIP reconciliation = 0 mismatches across all 290 ZIPs** (`zip_scores.total_gp_locations` vs actual `practice_locations` GP rows). New LIVE numbers (synced + read-back verified): per-LOCATION floor **261 / 4,811 GP = 5.43%** (CHI 242/4,449 = 5.44%, BOS 19/362 = 5.25%); NPI-level corporate **1,119 / 13,818 = 8.10%** — the long-stale Supabase `practices` table was healed surgically via `scrapers/_sync_practices_changed_rows.py` (per-row UPDATE, no TRUNCATE CASCADE), so **SQLite = Supabase on both legs now**. `consolidation-honesty.ts` `CONFIRMED_PER_DENTIST_CORPORATE` → IL **784/7,792 = 10.06%**; the band bar renders **5.43% → 10.06% → 14.6%** (per-LOCATION floor stays a runtime value from zip_scores). Frontend fixes in this audit: **(a)** `corporate_share_pct` / `buyable_practice_ratio` are stored as FRACTIONS (0.0543) — every display site now multiplies ×100 before formatting (market-intel, warroom + launchpad ZIP drawers, ownership-landscape, market-overview-charts, practice-density-map); **(b)** `da_unverified` added to `entity-classifications.ts` (rolls into "unknown" in `classifyPractice()`) and filtered at 10 query/display sites; **(c)** label honesty — freshness bar says "tracked locations" (was "federal NPI records"), Market Intel subtitle "locations incl. specialists" (was "NPI rows"), Job Market subtitle "GP location records"; **(d)** deals-by-source in `data-breakdown.ts` fixed (the column is `source`, NOT `data_source`; 5 sources incl. beckers) and `data-snapshot.ts` deal count refreshed (2,983 live). CI guards re-based in the parent repo: FLOOR expect_min 285→**261**, FLOOR_NPI 1178→**1119**. Full reconciliation: parent-repo `CLAUDE.md` 2026-06-12 dated note + F29 cheat-sheet.
...
- **4,811 GP clinic locations in watched ZIPs** (CHI 4,449 + BOS 362 — `SUM(zip_scores.total_gp_locations)`, post-2026-06-12 demotions + purge; was 4,970) — the location-deduped denominator after collapsing NPI-1 + NPI-2 + suite-variant rows at the same physical building, excluding the 179 `da_unverified` synthetic records. ~2.4× smaller than the raw NPI count. Surfaced as a subtitle on Home + Job Market "Total Practices" KPI cards (Phase A, commit `732894f`).
...
### All 13 Entity Classification Values
...
| `org_only_npi` | unknown | Organization NPI at an address with no practicing individual providers (billing/admin/closed). NPI-row only — never appears as a deduped location. Added 2026-05-30 |
| `da_unverified` | unknown | Data-Axle-only record (synthetic `DA_` NPI, no federal NPI at the address) — excluded from EVERY denominator and UI surface. Added 2026-06-12 |
